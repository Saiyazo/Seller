...
import { slotData } from "../data/DataAds";
...
const AdsOne = ({
  ...
  const [slotDataState, setSlotData] = useState(slotData);
  ...
  const handleDurationSelect = (slot, duration) => {
    ...
    setPrice(updatedSlot.prices[duration]);
  };
      ...
      <Row className="g-4">
        {slotDataState.map((slot) => (
          <Col xs={12} md={4} key={slot.id}>
            <Card
              className={`shadow-sm ${slot.availability === "Selected" ? "border-primary" : ""
                }`}
              style={{
                cursor:
            ...
            >
              ...
              <Card.Body>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (slot.availability === "Available") {
                            handleDurationSelect(slot, parseInt(duration));
                          }
                        }}
                      >
                ...
                <Button
                  ...
                  onClick={() => {
                    if (!slot.selectedDuration) {
                      alert("กรุณาเลือกระยะเวลาก่อนเลือกแพ็กเกจ");
                      return;
                    }
                    handleSlotSelect(
                      slot.id,
